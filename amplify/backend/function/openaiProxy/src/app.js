...
function App() {
  ...
  // We no longer need to initialize the OpenAI client here,
  // as the serverless function will handle the API call.
  ...
  const generateGames = async () => {
    ...
    try {
      ...
      // We are now calling our secure serverless function via Amplify's API.
      const restOperation = post({
          apiName: 'learn2gameapi', // This is the name of the API created by Amplify.
          path: '/generate-games', // This is the path we defined in our app.js
          options: {
            body: {
                lessonText: lessonText // Send the lesson text in the request body.
            }
          }
      });
  ...
  // --- Game Components (No changes needed here from before) ---
          ...
          <div key={bIndex} className="mb-8 p-4 border border-gray-200 rounded-lg bg-gray-50">
            ...
            {showResults[bIndex] && (
              <div className={`mt-4 p-3 rounded-md ${isCorrect(bIndex, b.answer) ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                ...
                <p className="mt-1 text-sm">Explanation: {b.explanation}</p>
                {b.hint && <p className="mt-1 text-sm italic">Hint: {b.hint}</p>}
              </div>
            )}
          </div>
        ))}
